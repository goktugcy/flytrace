...
function make(over: { sessions?: () => Promise<number>; links?: () => Promise<number> } = {}) {
  const calls = { sessions: 0, links: 0 };
...
describe('SecurityJanitor', () => {
  test('reaps expired sessions and stale link tokens', async () => {
    ...
    expect(await janitor.runOnce()).toEqual({ sessions: 3, linkTokens: 2 });
    expect(calls).toEqual({ sessions: 1, links: 1 });
  });
  ...
  test('a failing session reap does not prevent the link-token reap', async () => {
    ...
    expect(await janitor.runOnce()).toEqual({ sessions: 0, linkTokens: 2 });
  });
  ...
  test('a failing sweep never throws', async () => {
    ...
    expect(await janitor.runOnce()).toEqual({ sessions: 0, linkTokens: 0 });
  });
